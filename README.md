...
This repository contains the code for the a deep learning model used in the paper **“Automatic segmentation of the urethra and prostatic zones with deep learning”**. The model aim to automate the segmentation of the urethra, prostate, and prostatic zones on MRI scans, reducing the labor and variability associated with manual segmentation.
...
This repository includes the code for an nnU-Net [1]. The model was evaluated for its performance in segmenting the prostate, prostatic zones, and urethra on MRI by using the public PROSTATEx [2-4] dataset together with segmentations from the ProstateZones [5] dataset.

## Data Setup

To set up the data, follow the instructions provided at: https://github.com/UMU-DDI/ProstateZones . These instructions will guide you on how to download the images and segmentations and organize them into the required folder structure for running the model (as described below).
...
The training was performed with a Cross Entropy + Dice loss function, Stochastic Gradient Descent optimizer with Nesterov momentum (µ= 0.99), and on-the-fly data augmentation. The training ran for 1000 epochs for each fold in a five-fold cross-validation scheme with the 160 training samples. The initial learning rate were 0.01, with a weight decay of 3e-5.

Training and evaluation was performed on an NVIDIA GeForce RTX 3090 GPU.
...
Here, you have two options. Either, you can retrain the model from scratch (by following the steps specified under Usage Instructions on the nnU-Net page) or use our pretrained model weights. These are available at: https://drive.google.com/drive/folders/1-CboNLS5H_oOwQ7KPLtlVcLTWxeCwbNR?usp=sharing
...
For the simplest usage of the model, the trained weights are available at: https://drive.google.com/drive/folders/1-CboNLS5H_oOwQ7KPLtlVcLTWxeCwbNR?usp=sharing
...
To evaluate the performance of the model on the predicted data, Hero workflows are available.

*OutputSegmentations.ice*: creates images in Hero for the model. What is needed is the original image and the nnU-Net prediction in a folder. These outputs can then be saved to a Hero database.
...
[5] Holmlund W, Simkó A, Söderkvist K, Palásti P, Tótin S, Kalmár K, et al. ProstateZones – Segmentations of the prostatic zones and urethra for the PROSTATEx dataset. Scientific Data. 2024;11(1).
